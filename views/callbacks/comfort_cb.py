...
def _get_comfort_data(
    ...
    # NaN guard: forward-fill then zero-fill to prevent chart crashes
...
def _register_comfort_kpis(app: object) -> None:
    ...
    def update_comfort_kpis(
        ...
        _empty = [
            ...
            create_kpi_card("In Comfort Band", "—", icon="mdi:check-circle-outline"),
        ]
        ...
        try:
            ...
            if pd.isna(avg_temp) or pd.isna(avg_hum) or pd.isna(avg_co2):
                return _empty
            ...
            logger.warning(f"Comfort KPIs error: {e}")
            return _empty
...
def _register_comfort_temperature(app: object) -> None:
    ...
    def update_comfort_temperature(
        ...
        try:
            ...
            zone_ids = zones[:8] if zones else _monitored_zone_ids()[:8]
            ...
            for i, zid in enumerate(zone_ids):
                ...
                if zdf.empty or zdf["temperature_c"].isna().all():
                    continue
            ...
            logger.warning(f"Comfort temperature chart error: {e}")
    ...
    def update_comfort_matrix(
        ...
        try:
            ...
            for zid in zone_ids:
                ...
                row = []
                text_row = []
                for col, _label, band_key in metrics:
            ...
            logger.warning(f"Comfort matrix error: {e}")
            return empty_chart("Error loading chart")
...
def _register_comfort_co2_scatter(app: object) -> None:
    ...
    def update_co2_scatter(
        ...
        try:
            ...
            comfort_agg = (
                comfort_df.groupby(
                    [pd.Grouper(key="timestamp", freq="15min"), "zone_id"]
                )["co2_ppm"]
                .mean()
                .reset_index()
            )
            ...
            merged = comfort_agg.merge(
                occ_agg, on=["timestamp", "zone_id"], how="inner"
            )
            merged = merged.dropna(subset=["co2_ppm", "occupant_count"])
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers",
                    name="CO₂ readings",
                    marker=dict(size=4, opacity=0.4, color=ACCENT_BLUE),
                    hovertemplate=(
            ...
            logger.warning(f"CO₂ scatter error: {e}")
            return empty_chart("Error loading chart")
...
def _register_comfort_humidity(app: object) -> None:
    ...
    def update_humidity_violin(
        ...
        try:
            ...
            logger.warning(f"Humidity violin error: {e}")
            return empty_chart("Error loading chart")
